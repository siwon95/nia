...
$(document).ready(function() {
	...
	$.ajax({
		...
		dataType : "json",
		success:function(data){
			$("#depthButton").append("<li class='active'><a href='/site/${strDomain}/ex/bbs/List.do?cbIdx=1157'>전체</a></li>");
			$.each(data.cmmCode, function (index, item) {
				...
				if(searchDepth !=item.code){
					$("#depthButton").append("<li><a href='/site/${strDomain}/ex/bbs/List.do?cbIdx=1157&searchDepth="+item.code+"'>"+item.codeName+"</a></li>");
				}else{
					$("#depthButton").append("<li class='active'><a href='/site/${strDomain}/ex/bbs/List.do?cbIdx=1157&searchDepth="+item.code+"'>"+item.codeName+"</a></li>");
				}  
...
<form:hidden path="mode" />
	...
	<div class="board_list">
			...
			<c:if test="${bbsFVo.categoryUseYn eq 'Y'}">
				<form>
					<fieldset>
						...
						<div>
							<label for="txt1" class="soundOnly">검색어입력</label>
		...
		<div class="div_table b_type3 m_date">
			<div class="div_caption">인터넷윤리 게시물 목록</div>

...
public class EgovFormBasedUUID implements Serializable {
    ...
    public static EgovFormBasedUUID fromString(String name) {
        String[] components = name == null ? null : name.split("-");
    ...
    public int compareTo(EgovFormBasedUUID val) {
    	...
    	int ret = 0;
    	if(val != null){
    		ret = (this.mostSigBits < val.mostSigBits ? -1: (this.mostSigBits > val.mostSigBits ? 1: (this.leastSigBits < val.leastSigBits ? -1: (this.leastSigBits > val.leastSigBits ? 1 : 0))));
    	}
        return ret;
    }

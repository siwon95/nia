...
%>
	<li<%=activeClass%>><a href="<%=element1.getChild("user-menu-url").getText()%>"<%=activeClass%>><%=element1.getChild("menu-nm").getText()%></a>
<%
...
%>
			<li<%=activeClass%>><a href="<%=element2.getChild("user-menu-url").getText()%>"<%=activeClass%><%=linkType%>><%=element2.getChild("menu-nm").getText()%></a>								
<%
...
<%						
						for(Element element3:menuList3){
							sortOrder = (element3.getChild("sort-order").getText().toString()).trim();
							linkType = "";
							if(element3.getChild("link-type").getText().equals("popup")){
								linkType = " target=\"_blank\"";
							}
							activeClass = "";
							if(strSortOrder.substring(0,7).equals(sortOrder.substring(0,7))){
								activeClass = " class=\"active\"";
							}
%>
					<li<%=activeClass%>><a href="<%=element3.getChild("user-menu-url").getText()%>"<%=activeClass%><%=linkType%>><%=element3.getChild("menu-nm").getText()%></a></li>
<%						} %>
				</ul>
<%					} %>
			</li>

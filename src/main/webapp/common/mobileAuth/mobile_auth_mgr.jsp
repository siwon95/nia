...
if(retInfo.equals("")) {
	...
	String srvNo    = "001013";																// 본인실명확인 서비스번호
	String reqNum   = day + randomStr;														// 본인실명확인 요청번호
	String exVar    = "0000000000000000";													// 복호화용 임시필드
	String retUrl   = "32http://www.seocho.go.kr/common/mobileAuth/mobile_auth_mgr.jsp";	// 본인실명확인 결과수신 URL
	
	String platform	= EgovProperties.getProperty("Globals.PLATFORM");
	if(platform != null){
		if(!platform.equals("service")) {
			srvNo    = "015001";																	//localhost 용
			retUrl   = "32http://localhost:8080/common/mobileAuth/mobile_auth_mgr.jsp";				//localhost 용
		}
	}
	...
	try{
		...
		if(result != null){
			if(result.equals("Y")) {
				...
				out.println("	window.opener.writeJN('"+name+"', '"+di+"');");
		...
		System.out.println("[pcc] Receive Error - IOException");
		throw ex;
	}catch(Exception ex){
		System.out.println("[pcc] Receive Error - Exception");
		throw ex;
	}
}
%>
...
public class ConvUtils {
     ...
     * @return
     */
    public static <T> T getConvParam(HttpServletRequest request, Class<T> type)  throws Exception {
     ...
     * @return
     */
    public static <T> T getConvParam(HttpServletRequest request, Class<T> type, String filter) throws Exception {
            ...
            if(StringUtils.hasText(filter)) {
                ...
                if(isRequiredProperty(filter, field.getName())) {
                    if(value == null || (value instanceof String && !StringUtils.hasText((String) value)))  {
                        throw new Exception("Column[" + field.getName() + "] 는 요청[" + filter + "] 에 의해 필수값으로 설정되었습니다.");
                    }
                }
            }
     ...
     * @return
     */
    public static <T> List<T> getConvParamList(HttpServletRequest request, Class<T> type) throws Exception {
    ...
    public static <T> List<T> getConvParamList(HttpServletRequest request, Class<T> type, String filter) throws Exception {
            ...
            try {
                if (value != null) {
                    if(listSize != value.length) {
                        throw new Exception("요청에서 넘어온 데이터 셋의 사이즈가 상의합니다.");
                    } else {
                        for(int i=0; i<listSize; i++) {

	...
	<div class="portfolioDetail">
		...
		<div class="siteInfo">
			<ul>
				...
				<c:when test="${detailList.contentMapping eq 'EXT3'}">
				<li><span class="title">Details.</span> <c:out value="${detailList.dataContent }"/></li>
				</c:when>
				<c:when test="${detailList.contentMapping eq 'EXT4'}">
				<li class="noTitle"><span class="title">진행상태</span> <c:out value="${detailList.dataContent }"/></li>
				</c:when>
				<c:when test="${detailList.contentMapping eq 'EXT5'}">
				<li class="noTitle"><span class="title">프로그램 개발언어</span> <c:out value="${detailList.dataContent }"/></li>
				</c:when>
				<c:when test="${detailList.contentMapping eq 'EXT6'}">
				<li class="noTitle"><span class="title">프로젝트 성격</span> <c:out value="${detailList.dataContent }"/></li>
				</c:when>
				<c:when test="${detailList.contentMapping eq 'EXT7'}">
				<li class="noTitle">
					<span class="title">URL</span> 
					<a href="<c:out value="${detailList.dataContent }"/>" target="_blank" title="새창열림"><c:out value="${detailList.dataContent }"/></a> &nbsp;&nbsp;
					<a href="<c:out value="${detailList.dataContent }"/>" target="_blank" title="새창열림" class="btn_inline"><img src="/images/sub/btn_portfolioLink.png" alt=""> Launch Site</a>
				</li>
			</ul>
		</div>
		<div class="portfolioVisual">
			...
			<span class="thumb"><img src="/common/board/Download.do?bcIdx=<c:out value="${fileList.bcIdx}"/>&amp;cbIdx=<c:out value="${fileList.cbIdx}"/>&amp;streFileNm=<c:out value="${fileList.streFileNm}"/>" alt="섬네일이미지(<c:out value="${fileList.streFileNm}"/>)"></span>
							</c:if>
						</c:forEach>
					</c:if>
					<c:if test="${prevNextList.PREV_IDX ne null}">
						<a href="#" onclick="doPrevNext('<c:out value="${bbsFVo.cbIdx }"/>', '<c:out value="${prevNextList.PREV_IDX }"/>');return false;" class="btn_prev">이전글</a>
					</c:if>
					<c:if test="${prevNextList.NEXT_IDX ne null}">
						<a href="#" onclick="doPrevNext('<c:out value="${bbsFVo.cbIdx }"/>', '<c:out value="${prevNextList.NEXT_IDX }"/>');return false;" class="btn_next">다음글</a>
					</c:if>
			<a href="#" class="btn_next">다음글</a>
			<a href="#" class="btn_prev disabled">이전글</a>
		</div>
				</c:when>

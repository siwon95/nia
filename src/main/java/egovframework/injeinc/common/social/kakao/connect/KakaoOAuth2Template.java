...
public class KakaoOAuth2Template extends OAuth2Template {
	...
	protected AccessGrant postForAccessGrant(String accessTokenUrl, MultiValueMap<String, String> parameters) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug(new StringBuilder("accessTokenUrl : ").append(accessTokenUrl).toString());
			logger.debug(new StringBuilder("requestEntity : ").append(requestEntity).toString());
		}
		if (logger.isDebugEnabled()) {

...
//<![CDATA[
	...
	function doSearch() {
		...
		$("#UserVo").attr("action", "<c:url value='/user/UserSearchListPop2.do' />").submit();
		
	}
	
	function selectUser(cuId,cuName,cuHp) {
		window.opener.focus();
		window.opener.selectUser(cuId,cuName,cuHp);
		self.close();
	}
		...
		<table summary="아이디와 비밀번호로 회원정보 검색" class="view">
			...
			<tbody>
				...
				<td>
					...
					<a href="javascript:doSearch();" class="btn2" id="ida">검색</a>					
					</span>
				</td>
			</tbody>
		</table>
		...
		<table summary="회원 목록" class="list1">
			...
			<tbody>
				...
				<tr>
					...
					<td><a href="javascript:selectUser('<c:out value="${result.cuId}"/>','<c:out value="${result.cuName}"/>','<c:out value="${result.coNum}"/>');" class="btn">선택</a></td>
				</tr>
				</c:forEach>

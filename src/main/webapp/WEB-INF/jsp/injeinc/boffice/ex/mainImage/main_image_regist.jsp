...
function fncMainImageInsert() {
    ...
    var varFrom = document.getElementById("mainImage");
    varFrom.action = "/boffice/ex/mainImage/mainImageReg.do";
...
<body>
	...
	<input type="hidden" name="ntceEndde" value="<c:out value='${mainImage.ntceEndde}'/>">
		...
		<table summary="" class="write">
			...
			<tr>
				...
				<td>
					...
					<c:forEach var="result" items="${rowDataList }" varStatus="status">
						<td>
							<input type="checkbox" name="type" id="<c:out value="${result.code }"/>" value="<c:out value="${result.code }"/>"/> <label for="<c:out value="${result.code }"/>"><c:out value="${result.codeName }"/></label>
						</td>
			...
			<tr>
				...
				<td>
				    ...
				    &nbsp;&nbsp;~&nbsp;&nbsp;
				    <label for="ntceEnddeYYYMMDD" class="hidden">게시마감날짜</label>
			...
			<tr>
				...
				<td>
				    ...
				    &nbsp;&nbsp;&nbsp;&nbsp;
				    <input type="checkbox" id="newWindowAt" name="newWindowAt" value="Y" <c:if test="${mainImage.newWindowAt == 'Y'}">checked</c:if> /> <label for="newWindowAt">새창 띄우기</label>
				</td>
			</tr>
			<tr>
			  	<th>* <label for="egovComFileUploader">이미지 첨부</label></th>
			  	<td>
			    	<input type="file" name="file_1" id="egovComFileUploader" title="이미지" onchange="javascript:fncOnChangeImage(this);"/>
			  	</td>
			</tr>
			<tr>
			...
			<tr>
				<th>* <label for="reflctYnY">게시상태</label></th>
				<td>
		...
		<div class="btn_zone">
			...
			<%-- 저장 --%>
			<a href="javascript:fncMainImageInsert();" class="btn2">저장</a>
		</div>
		<!--// 버튼 -->

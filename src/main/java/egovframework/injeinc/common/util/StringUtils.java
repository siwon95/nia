...
/**
     ...
     * @return
     */
    public static void checkPassword(String pw) throws Exception {
        byte b[] = pw.getBytes();
        int sc[] = {0, 0, 0};
        ...
        for(int i = 0; i < b.length; i++){
            ...
            if(sc[0] >= 3 || sc[1] >= 3 || sc[2] >= 3){
                throw new Exception("같은문자가 반복되거나 연속되는 문자가 중복됩니다.(4회이상연속)");
            }
        }
        ...
        for(int i = 0; i < pw.length()-3; i++){
            ...
            if("qwertyuiop".indexOf(temp) >= 0){
                throw new Exception("키보드상의 배열상 연속되는 문자는 사용하실 수 없습니다.(4회이상연속)");
            }
        ...
        for(int i = 0; i < pw.length()-5; i++) {
            ...
            if(isnum){
                ...
                if(mm == 1 || mm == 3 || mm == 5 || mm == 7 || mm == 8 || mm == 10 || mm == 12) {
                    if(dd <= 31){
                        throw new Exception("날짜형식으로 확인되는 문자는 사용하실 수 없습니다.");
                    }
                }

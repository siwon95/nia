	...
	$.ajax({
		...
		success:function(data){
			...
			$.each(data.cmmCode, function (index, item) {
				console.log("item.codeValue : "+item.codeValue);
	 ...
	 $.ajax({
		...
		dataType : "json", 
		success:function(data){
			$("#centerButton").append("<li class='active'><a href='/site/${strDomain}/ex/bbs/List.do?cbIdx=1118'>전체</a></li>");
			$.each(data.cmmCode, function (index, item) {
				var searchCenter = '${bbsFVo.searchCenter}';
				if(searchCenter !=item.code){
					$("#centerButton").append("<li><a href='/site/${strDomain}/ex/bbs/List.do?cbIdx=1118&searchCenter="+item.code+"'>"+item.codeName+"</a></li>");
				}else{
					$("#centerButton").append("<li class='active'><a href='/site/${strDomain}/ex/bbs/List.do?cbIdx=1118&searchCenter="+item.code+"'>"+item.codeName+"</a></li>");
				}    
	    		});
	...
	<div class="board_list">
		<div class="board_search">
			<fieldset>
				...
				<div>
					<label for="아이디값1">주소</label>
				 	<input type="text" name="searchKey" id="searchKey" value="<c:out value='${bbsFVo.searchKey}' />" title="검색어입력" placeholder="검색어를 입력해주세요">
		...
		<div class="div_table b_type4">
			<div class="div_caption">관계기관 게시물 목록</div>

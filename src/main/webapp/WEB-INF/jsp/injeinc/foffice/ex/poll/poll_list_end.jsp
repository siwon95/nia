...
//<![CDATA[
	...
	function doPollListF() {
		$("#pageIndex").val("1");
		$("#PollListFVo").attr("action", "<c:url value='/site/${strDomain}/ex/poll/PollListE.do' />").submit();
	}
	
	function doPollListPag(pageIndex) {
		$("#pageIndex").val(pageIndex);
		$("#PollListVo").attr("action", "<c:url value='/site/${strDomain}/ex/poll/PollListE.do' />").submit();
	}
	...
	<table class="list">
		...
		<tbody>			
			...
			<tr>
				...
				<td class="subject"><a href="/site/<c:out value="${strDomain}" />/ex/poll/PollListFViewEnd.do?plIdx=${result.plIdx}"><c:out value="${result.plTitle}" /></a></td>
				...
				<td>
					...
					<c:choose>
						<c:when test="${result.plResultOpenYn eq 'Y'}">
							<a href="#none" onclick="doPollListFReport('<c:out value="${result.plIdx}" />'); return false;" class="btn-inner">설문마감<br/>(결과보기)</a>
						</c:when>
						<c:otherwise>
							<span style="color:red;">설문마감<br/>(비공개)</span>
						</c:otherwise>
					</c:choose>					
				</td>
			</tr>

	...
	$.ajax({
		...
		, success : function(data) {
			...
			$(data.secondSelectList).each(function(index, element){
				output += "<option value="+element.cbCd+" camel="+element.cbIdx+" temgbn="+element.bbsTempletGbn+">"+ element.cbName +"</option>";
			});
...
<input type="hidden" name="cbIdx" id="cbIdx" />
					...
					<td>
						...
						<select name="startTime" id="startTime" style="width: 80px;">
							...
							<c:forEach items="${timeSelect}" var="timeSelect" varStatus="status">
								<option value="<c:out value="${timeSelect.colValue}"/>" <c:if test="${timeSelect.colValue eq startTime}">selected="selected"</c:if>><c:out value="${timeSelect.colValue}"/></option>
							</c:forEach>
						...
						<select name="endTime" id="endTime" style="width: 80px;">
							<option value="">:: 선택 ::</option>
							<c:forEach items="${timeSelect}" var="timeSelect" varStatus="status">
								<option value="<c:out value="${timeSelect.colValue}"/>" <c:if test="${timeSelect.colValue eq endTime}">selected="selected"</c:if>><c:out value="${timeSelect.colValue}"/></option>
							</c:forEach>
						</select>시
					...
					<tr class="<c:out value="${contentList.NOTI_YN eq 'Y'?'notice':'none'}"/>">
						...
						<c:forEach items="${contMappList}" var="contMappList">
								<c:choose>
									...
									<c:when test="${contMappList.CONTENT_MAPPING eq 'REGDT_CONT'}"><td class="none"><c:out value="${contentList.REGDT_CONT}" /></td></c:when>

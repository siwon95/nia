	...
	public class WNSearch {
		...
		*/
		...
		public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
			...
			sbRet.append("<div class='board-foot-container'>");
			sbRet.append("	<div class='pagination'>");
			if(wnanchor.getBundleBefore() != -1) {
				sbRet.append("<a class='prev' href=\"#none\" onClick=\"javascript:doPaging('"+wnanchor.getBundleBefore()+"');\" ><<</a>&nbsp;&nbsp;");
			} else {
// 				sbRet.append("<a class='prev' href=\"#none\" title=\"이전페이지\"><</a>&nbsp;&nbsp;");
			} 
			if(wnanchor.getBefore() != -1) {
				sbRet.append("<a class='back' href=\"#none\" onClick=\"javascript:doPaging('"+wnanchor.getBefore()+"');\" title=\"이전페이지\"><</a>&nbsp;&nbsp;");
			} else {
// 				sbRet.append("<a class='back' href=\"#none\" title=\"이전페이지\"><</a>&nbsp;&nbsp;");
			} 
			...
			for(int i=0; i<pageCount && i < pages.length; i++) {
				if(pages[i][1].equals("-1")) {
					sbRet.append("<span class='pg'>" + pages[i][0] + "</span>");
				} else {
				...
				count++;
					...
					sbRet.append("&nbsp;&nbsp;");
			if(wnanchor.getNext() != -1) {
    			sbRet.append("<a class=\"forward\" href=\"#none\" onClick=\"javascript:doPaging('"+wnanchor.getNext()+"')\" title=\"다음페이지\">></a>");
			} else {
// 				sbRet.append("<a class=\"forward\" href=\"#none\" title=\"다음페이지\">></a>");
			} 
			if(wnanchor.getBundleNext() != -1) {
				sbRet.append("<a href='#none' class='next' onClick=\"javascript:doPaging('"+wnanchor.getBundleNext()+"')\" >>></a>");
			}else{
// 				sbRet.append("<a href='#none' class='next' onClick=\"javascript:doPaging('"+wnanchor.getLastPage()+"')\"  >>>"+pages[pageCount-1][1]	+"</a>");
			}
				sbRet.append("	</div>");
				sbRet.append("</div>");

	...
	<dl>
		...
		<c:forEach items="${contentList}" var="contentList" varStatus="status">
			<dt><a href="#">
				...
				<c:if test="${not empty contentList.SUB_CONT}"><span class="title"><c:out value="${fn:replace(fn:replace(fn:replace(contentList.SUB_CONT,'&','&amp;'),'<','&lt;'),'>','&gt;')}" escapeXml="false"/></span></c:if>
			</a></dt>
			<dd>
				<c:if test="${not empty contentList.CLOB_CONT_SEARCH}"><c:out value="${contentList.CLOB_CONT_SEARCH}" escapeXml="false" /></c:if>
			</dd>
		</c:forEach>
	</dl>
</div>
